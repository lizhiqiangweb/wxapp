			...
			<view class="memberInfo-top flex">
				...
				<view class="memberInfo-info">
					<view class="memberInfo-name flex">
						{{memberName}}
						<text v-if="memberInfo.village_name!=='' && memberInfo.village_name!==undefined">| {{memberInfo.village_name}}</text>
					</view>
					<text class="identity" v-if="memberInfo.identity!=='' && memberInfo.identity!==undefined">{{memberInfo.identity}}</text>
				</view>
			</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getMyHomeInfo() {
				let that = this
				that.$request.httpTokenRequest(that.$api.getTargetHomeInfo, that.$api.get, {
					Key: that.$api.Key,
					Client: that.$api.CLIENT,
					Version: that.$api.VERSION,
